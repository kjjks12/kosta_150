 ...
 *fk(외래키)를 별도로 설정하는 방법
 ...
 ------------------------------------------------------------
  ...
  3)  unique
       => null은 허용하나 중복안됨(not null을 지정하면 pk동일)
       => 한테이블에 여러개의 컬럼이 unique 가능
       
  4) check
       => 조건을 설정하여 insert할때 조건이외의 값을 삽입하지 
          못하도록 하는 것.
 
  5) default
      => 기본값을 설정해서 자동으로 기본값이 삽입되도록 하는것.


 ex) 테이블생성
 create table test(
   id varchar2(10)  unique, --중복안됨, null허용
   jumin char(13) not null unique, --중복안됨, null안됨.
   age number(2) check (age  between 10 and 20),
   gender char(2 char)  default '남자' not null --default를 설정할때 not null함께 사용하면 default를 먼저 작성한다.
 );
...
insert into test(jumin,age) values('222',16);--오류 jumin 중복안됨
insert into test(jumin,age) values('33',25); --오류 age 범위
...
-------------------------------------------------------
 -- 테이블수정(구조 변경)
 1) 컬럼 추가
  alter table 테이블이름 add 컬럼명 자료형 제약조건 , 컬럼명 .....
  
  alter table test add etc varchar2(20) ;
  alter table test add etc2 varchar2(20) default '기타' not null;
...
alter table emp add constraint emp_unique unique(addr);
 ...
 6) 컬럼 이름 변경
 alter table 테이블이름 rename column 기존컬럼이름 to 변경컬럼이름;
